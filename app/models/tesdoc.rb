...
#      rigdoc.prezzo     = art.prezzo * (1 - rigdoc.sconto / 100)
  ...
  def self.filter (nrini, nrfin, dtini, dtfin, tp, des, tpc, tipo_doc, causmag, conto, azienda, annoese, page)
    ...
    whtesdoc = ""
    whtesdoc += " and tesdocs.data_doc >= :dti" if dtini
    hshvar[:dti] = dtini if dtini
    whtesdoc += " and tesdocs.data_doc <= :dtf" if dtfin
    hshvar[:dtf] = dtfin if dtfin
    whtesdoc += " and tesdocs.num_doc >= :nri" if nrini
    hshvar[:nri] = nrini if nrini
    whtesdoc += " and tesdocs.num_doc <= :nrf" if nrfin
    hshvar[:nrf] = nrfin if nrfin
    ...
    nrrecord = includes(:causmag, :conto =>[:anagen]).where([whcausmag + whconto + whana + whtesdoc, hshvar]).azdanno(
      azienda, annoese).count if page.nil?

    return includes(:causmag, :conto =>[:anagen]).
             where([whcausmag + whconto + whana + whtesdoc, hshvar]).
  ...
  def self.art_mov_vend(idart, idanagen, nrmag, anarif, azienda, tp, annoese, dti, dtf, idcausmag)
    # Documenti che movimentano uno o piu' articoli per uno o piu' conti per uno o piu' magazzini
    if idart == "all"
      where_art = " AND articles.azienda = #{azienda}"
    else
      where_art = " AND articles.id = #{idart}"
    end
    #idart == "all" ? where_art = " WHERE articles.azienda = #{azienda}" : where_art = " WHERE articles.id = #{idart}"

    where_tesdoc=""
    if dti || dtf
      where_tesdoc+=" AND tesdocs.data_doc >= '#{dti}'" if dti
      where_tesdoc+=" AND tesdocs.data_doc <= '#{dtf}'" if dtf
    end
    ...
    Tesdoc.find_by_sql("SELECT DISTINCT articles.id AS artid, articles.codice AS codice
                         ...
                         WHERE tesdocs.annoese = #{annoese} #{where_tesdoc}
                         #{where_art} #{filter_causmag} #{filter_anagen} #{filter_tpmov} #{filter_nrmag} ORDER BY articles.codice")
  end
  
  def self.anagen_mov_artic(idart, idanagen, nrmag, anarif, tp, annoese, dti, dtf, idcausmag)
    ...
    filter_tpmov = " AND causmags.tipo IN ('U','E','V','R')"
    if tp == "M"
      where_causmag = "  AND causmags.movimpmag IN ('M', 'I') "
      if nrmag == ""
        filter_nrmag = " AND (tesdocs.nrmagsrc > 0 OR tesdocs.nrmagdst > 0)"
      else
        filter_nrmag = " AND (tesdocs.nrmagsrc = #{nrmag} OR   tesdocs.nrmagdst = #{nrmag})"
      end
    else
      where_causmag = " AND causmags.contabile = 'S' "
      filter_nrmag = ""
    end
  ...
  def self.mag_mov_artic_anagen(idart, idanagen, nrmag, anarif, grpmag, tp, annoese, dti, dtf, idcausmag)
    ...
    if nrmag == ""
      filter_nrmagsrc = " AND tesdocs.nrmagsrc > 0"
      filter_nrmagdst = " AND tesdocs.nrmagdst > 0"
    else
  ...
  def self.mov_artanagenmag(idart, idanagen, nrmag, anarif, grpmag, annoese, dti, dtf, idcausmag)
  ...
  def self.ven_artanagen(idart, idanagen, anarif, annoese, dti, dtf, idcausmag)
